...
//http://localhost:5001/auth?token=tktk9wv7I8UU26FGGhtsSyMgZv8caqygNgPVMrdDw02IZlnRhbK3s&username=DarkRaimbos
express()
  .use((req, res, next) => 
  ...
  .get("/start", asyncHandler(async (req, res) => {
	req.session['mmm'] = await startPuppeteerSession();
    res.sendStatus(200);
  }))
  ...
  .get("/auth", asyncHandler(async (req, res) => {
    try{
		...
		const page = await req.session['mmm'].page
